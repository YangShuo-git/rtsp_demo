...
int rtp_payload_check(struct rtp_payload_helper_t* helper, const struct RtpPacket* pkt)
{
    ...
    if (-1 == helper->__flags)
    {
        ...
        helper->__flags = 0;
        helper->seq = (uint16_t)(pkt->header.seq - 1); // disable packet lost
        helper->timestamp = pkt->header.timestamp + 1; // flag for new frame
    }

    lost = 0;
    // check sequence number
    if ((uint16_t)pkt->header.seq != (uint16_t)(helper->seq + 1))
    {
        lost = 1;
        //helper->size = 0;
        helper->lost = 1;
        //helper->flags |= RTP_PAYLOAD_FLAG_PACKET_LOST;
        //helper->timestamp = pkt->header.timestamp;
    }
    helper->seq = (uint16_t)pkt->header.seq;

    // check timestamp
    if (pkt->header.timestamp != helper->timestamp)
    {
 ...
 * @return
 */
int rtp_payload_write(struct rtp_payload_helper_t* helper, const struct RtpPacket* pkt)
{
    int size;
