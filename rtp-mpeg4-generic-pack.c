...
static void* rtp_mpeg4_generic_pack_create(int size, uint8_t pt, uint16_t seq, uint32_t ssrc, struct rtp_payload_t *handler, void* cbparam)
{
    ...
    packer->size = size;
    packer->pkt.header.v = RTP_VERSION;    // 版本号一定是2
    packer->pkt.header.pt = pt;        // payload type
    packer->pkt.header.seq = seq;      // 起始的sequence
...
static int rtp_mpeg4_generic_pack_input(void* pack, const void* data, int bytes, uint32_t timestamp)
{
    ...
    struct rtp_encode_mpeg4_generic_t *packer;
    packer = (struct rtp_encode_mpeg4_generic_t *)pack;
    packer->pkt.header.timestamp = timestamp; //(uint32_t)(time * KHz);
    ...
    // 担心AAC帧可能要拆分多个RTP包
    for (size = bytes; 0 == r && bytes > 0; ++packer->pkt.header.seq)
    {
        ...
        packer->pkt.payloadlen = (bytes + N_AU_HEADER + RTP_FIXED_HEADER_LEN) <= packer->size ?     // 判断是否要划分多个AU
                    bytes : (packer->size - N_AU_HEADER - RTP_FIXED_HEADER_LEN);
        ptr += packer->pkt.payloadlen;
        bytes -= packer->pkt.payloadlen;        // 剩余的字节数

        n = RTP_FIXED_HEADER_LEN + N_AU_HEADER + packer->pkt.payloadlen;
        ...
        packer->pkt.header.m = (0 == bytes) ? 1 : 0;       // 最后一个AU mark设置为1,否则为0
